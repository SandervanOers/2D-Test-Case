...
static double Hamiltonian(Vec Sol, double DeltaX, Vec R0);
...
static double Exact_Solution_m(double x, double t, double N2, double sigma, unsigned int k)
{
    return exp(-0.5*N2*x)*sin(2.0*PETSC_PI*k*x)*sin(2.0*PETSC_PI*sigma*t);
}
static double Exact_Solution_p(double x, double t, double N2, double sigma, unsigned int k)
{
    return exp(-0.5*N2*x)*(N2/4.0/sigma/PETSC_PI*sin(2.0*PETSC_PI*k*x)+k/sigma*cos(2.0*PETSC_PI*k*x))*cos(2.0*PETSC_PI*sigma*t);
}
static double Inital_Condition_m(double x1, double x2, double t, double N2, double sigma, unsigned int k)
{
    // int m dx = -(sin(2*pi*sigma*t)*exp(-(N2*x)/2)*((N2*sin(2*pi*k*x))/2 + 2*k*pi*cos(2*pi*k*x)))/(N2^2/4 + 4*pi^2*k^2)
    return -(exp(-(N2*x2)/2.0)*sin(2.0*PETSC_PI*sigma*t)*((N2*sin(2.0*PETSC_PI*k*x2))/2.0 + 2.0*k*PETSC_PI*cos(2.0*PETSC_PI*k*x2)))/sigma/sigma+(exp(-(N2*x1)/2.0)*sin(2.0*PETSC_PI*sigma*t)*((N2*sin(2.0*PETSC_PI*k*x1))/2.0 + 2.0*k*PETSC_PI*cos(2.0*PETSC_PI*k*x1)))/sigma/sigma;
}
static double Inital_Condition_p(double x1, double x2, double t, double N2, double sigma, unsigned int k)
{
    ...
    //return -(exp(-(N2*x2)/2.0)*(N2*N2*sin(2.0*PETSC_PI*k*x2) - 16.0*k*k*PETSC_PI*PETSC_PI*sin(2.0*PETSC_PI*k*x2) + 8.0*N2*k*PETSC_PI*cos(2.0*PETSC_PI*k*x2)))/(sigma*(4.0*sigma*sigma))+(exp(-(N2*x1)/2.0)*(N2*N2*sin(2.0*PETSC_PI*k*x1) - 16.0*k*k*PETSC_PI*PETSC_PI*sin(2.0*PETSC_PI*k*x1) + 8.0*N2*k*PETSC_PI*cos(2.0*PETSC_PI*k*x1)))/(sigma*(4.0*sigma*sigma));
    return -(exp(-(N2*x2)/2.0)*cos(2.0*PETSC_PI*sigma*t)*(N2*N2*sin(2.0*PETSC_PI*k*x2) - 16.0*k*k*PETSC_PI*PETSC_PI*sin(2*PETSC_PI*k*x2) + 8.0*N2*k*PETSC_PI*cos(2.0*PETSC_PI*k*x2)))/(2.0*sigma*PETSC_PI*(N2*N2 + 16.0*PETSC_PI*PETSC_PI*k*k)) + (exp(-(N2*x1)/2.0)*cos(2.0*PETSC_PI*sigma*t)*(N2*N2*sin(2.0*PETSC_PI*k*x1) - 16.0*k*k*PETSC_PI*PETSC_PI*sin(2.0*PETSC_PI*k*x1) + 8.0*N2*k*PETSC_PI*cos(2.0*PETSC_PI*k*x1)))/(2.0*sigma*PETSC_PI*(N2*N2 + 16.0*PETSC_PI*PETSC_PI*k*k));
}
static double Background_Density(double x, double N2)
{
    double b00 = 1.0;
...
int main(int argc,char **args)
{
  ...
  PetscInt      Number_Of_Elements=10, Number_Of_TimeSteps_In_One_Period=10;
  //PetscScalar   Number_Of_Periods=1;
  ...
  PetscScalar   sigma, N2 = 1.0;
  PetscInt      k = 1;
  PetscScalar   DeltaX = 1.0/(double)Number_Of_Elements, DeltaT=1.0/(double)Number_Of_TimeSteps_In_One_Period;
  ...
  double DeltaXOld = DeltaX;
  DeltaX = 1;
  // Interior Points
  for (i=1; i<Number_Of_Elements-1; i++)
  {
    // U Equations
    MatSetValue(P, i, i, 1, INSERT_VALUES);
    ...
    // P Equations
    MatSetValue(P, Number_Of_Elements+i, Number_Of_Elements+i, 1, INSERT_VALUES);
    PetscScalar rho0[3];
  ...
  // Left Boundary
  MatSetValue(P, 0,0, 1, INSERT_VALUES);
  ...
  PetscScalar rho02[2];
  //PetscInt ixx[2] = {Number_Of_Elements-1, Number_Of_Elements};
  PetscInt ixx[2] = {Number_Of_Elements-2, Number_Of_Elements-1};
  PetscPrintf(PETSC_COMM_WORLD, "2.1\n");
  VecGetValues(R0,2,ixx,rho02);
  PetscPrintf(PETSC_COMM_WORLD, "2.2\n");
  ...
  double Initial_Energy = Hamiltonian(Initial_Condition, DeltaX, R0);
  ...
  int t;
  for (t=0;t<Number_Of_TimeSteps_In_One_Period*Number_Of_Periods;t++)
  {
    MatMult(Q, Sol, RHS);
    KSPSolve(ksp,RHS,Sol);
    //KSPView(ksp,PETSC_VIEWER_STDOUT_WORLD);
    //VecCopy(Sol, RHS);
    Energy = Hamiltonian(Sol, DeltaX, R0);
