...
/*--------------------------------------------------------------------------*/
extern void Simulate(const Mat &A, const Mat &B, const Mat &M1_small, const Vec &Initial_Condition, const std::vector<Squares2D> &List_Of_Elements, const unsigned int &N_Nodes, const unsigned int &Number_Of_TimeSteps, const double &DeltaT, Vec &Sol, const unsigned int &Number_Of_Variables)
{

    double H0 = calculate_Hamiltonian2D_full(M1_small, Initial_Condition, List_Of_Elements, N_Nodes);
    ...
    for (unsigned int t = 0; t < Number_Of_TimeSteps; t++) //
    {
        ...
        time = (t+1)*DeltaT;
            ...
            H1 = calculate_Hamiltonian2D_full(M1_small, Sol, List_Of_Elements, N_Nodes);
...
/*--------------------------------------------------------------------------*/
extern double calculate_Hamiltonian2D_full(const Mat &M1, const Vec &Solution, const std::vector<Squares2D> &List_Of_Elements, const unsigned int &N_Nodes)
{
    double H = 0.0;
    PetscScalar *XTemp;
    VecGetArray(Solution, &XTemp);
