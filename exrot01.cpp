...
MatView(EToV, viewer);
std::vector<std::unique_ptr<Element>> List_Of_Elements2;
std::vector<std::unique_ptr<Vertex>>   List_Of_Vertices2;
load_msh_mesh(mesh_name, EToV2, List_Of_Vertices2, List_Of_Elements2, element_num,  node_num);
std::cout << "EToV2 = " << std::endl;
MatView(EToV2, viewer);

print(List_Of_Vertices2);
...
create_rot_WA_System_Forced_MidPoint(E, ET, invM, invM_small, M1, M1_small, M2, M2_small, NMat, NDerivMat, Forcing_a, N_Nodes, N_Petsc, DeltaT, nu, A, B, DIV, Re, Fr, gamma);
...
Vec VecNodes;
compute_InitialCondition_rot_WA(List_Of_Elements, N_Nodes, rho_0_Deriv, Fr, kxmode, kzmode, Initial_Condition, VecNodes, Number_Of_Elements_Petsc, Number_Of_TimeSteps_In_One_Period, N_Petsc, DIV);
...
Simulate_rot(A, B, M1_small, M2_small, DIV, Initial_Condition, VecNodes, List_Of_Elements, N_Nodes, Number_Of_TimeSteps, DeltaT, Sol, 3, F0, omega);
MatDestroy(&DIV);
