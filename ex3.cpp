...
int main(int argc,char **args)
{
    ...
    //MatSetValues(Fscale, 1, jx, Number_Of_Elements, ix, row_min, INSERT_VALUES);
    //x[0]=1;
    //MatSetValues(Fscale, 1, jx, Number_Of_Elements, ix, row_max, INSERT_VALUES);
    ...
    //MatView(EtoEF.EToE, PETSC_VIEWER_STDOUT_SELF);
    //MatView(EtoEF.EToF, PETSC_VIEWER_STDOUT_SELF);
    ...
    {
        double value = i;//(double)Initial_Condition_m((double)i*DeltaX, (double)(i+1.0)*DeltaX, 0, N2, sigma, k)/DeltaX;
        VecSetValue(Initial_Condition,i,value, INSERT_VALUES);
        VecSetValue(VecU,i,value, INSERT_VALUES);
        value = 1/(1.0+i);//(double)Initial_Condition_p((double)i*DeltaX, (double)(i+1.0)*DeltaX, 0, N2, sigma, k)/DeltaX;
